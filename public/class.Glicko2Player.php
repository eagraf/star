<?php
/*******************************************************************************
Modified implementation of Glicko2
Modified by Ethan Graf on 11/28/2015
Taken from https://github.com/Zarel/glicko2-php/blob/master/class.Glicko2Player.php
This version assumes that players are not persistently stored in memory, and instead can be reaccessed from a database.
********************************************************************************
...
Usage
Glicko2Player($type, $id, [$rating = 1500 [, $rd = 350 [, $volatility = 0.06 [, $mu [, $phi [, $sigma [, $systemconstant = 0.75 ]]]]]]]
...
class Glicko2Player {
	...
	var $M = array();
	function __construct($type, $id, $rating = 1500, $rd = 350, $volatility = 0.06, $mu = null, $phi = null, $sigma = null, $systemconstant = 0.75) {
		
		$this->type = $type;
		$this->id = $id;
		
		// Step 1:
		// Determine an initial rating and deviation.
		$this->rating = $rating;
		$this->rd = $rd;
		// volatility
		if (is_null($sigma)) {
		...
		// Step 2
		// Rating
		// Converting from the original Glicko scale to the Glicko-2 scale.
		if (is_null($mu)) {
